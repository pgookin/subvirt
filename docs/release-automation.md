...
Provider-only candidates default to `UBUNTU_TARGETS=all`, `ALMA_TARGETS=all`, and `LAB_TARGETS=all`. They build provider packages for every supported target, create fresh ephemeral lab VMs for every configured Ubuntu/Alma release, install patched libvirt and virt-manager from the current stable repo, install the staged provider package from the per-run lab repo, and run the TrueNAS storage gate for each target. Same-release migration peers are used when `tests.run_migration=true`. Provider-only candidates require `lab.enabled=true`, `truenas.api_key`, and target VM entries in the local lab config. Each candidate writes `artifacts/<build-id>/candidate-release.log` and must pass `scripts/verify-release-evidence.py` before stable publishing. Set `PROMOTE_STABLE=true` only when that provider build should be published to the public stable repository; Codex review is advisory by default and becomes blocking only with `REQUIRE_CODEX_GATE=true`.

Upstream libvirt refresh candidates keep `LAB_TARGETS=selected` by default. That means a U24-only refresh deep-tests U24, an AL9-only refresh deep-tests AL9, and a multi-target refresh deep-tests only the selected targets. The selected targets are derived from the upstream report plus any baseline targets intentionally added by `scripts/lock-release-inputs.py`.
...
`test-artifacts` installs packages directly from the build VM artifact directory before anything is published. The legacy direct-artifact path still defaults to Ubuntu 24.04 and AlmaLinux 10 test hosts. The ephemeral lab path is target-aware: Ubuntu artifacts are stored per suite under `/srv/subvirt/artifacts/<build-id>/ubuntu/<suite>`, Alma artifacts are stored per major version under `/srv/subvirt/artifacts/<build-id>/alma/<version>`, and each selected VM installs from the matching staging repo path. The lab then restarts the provider and libvirt storage daemon, runs `doctor --json`, verifies the `truenas` storage backend is visible, and runs the storage gate for each selected target.
...
Stable publishing requires a build ID. Before updating active package metadata, the publisher copies publishable `.deb` files and non-source/non-debug `.rpm` files to an immutable archive at `/archive/builds/<build-id>/artifacts/` and writes `/archive/builds/<build-id>/manifest.json` with package paths, sizes, and SHA-256 checksums. Reusing the same build ID with identical artifacts is idempotent; reusing it with different artifacts fails.

The active stable repo is pruned during publish. Apt pools keep only the incoming version for package names present in the current publish and remove libvirt packages from the wrong Ubuntu suite pool. Yum repos keep only the incoming version for package/arch pairs present in the current publish and remove RPMs from the wrong EL major. Packages not present in the current publish are preserved so provider-only releases do not remove the matching libvirt package set. Historical packages belong in `/archive`, not the active apt/dnf repo.

The published layout is:

- `https://repo.subvirt.net/apt/ubuntu/dists/{bionic,focal,jammy,noble,resolute}/stable/...`
- `https://repo.subvirt.net/apt/ubuntu/pool/stable/{bionic,focal,jammy,noble,resolute}/...`
- `https://repo.subvirt.net/yum/almalinux/{9,10}/stable/...`
- `https://repo.subvirt.net/archive/builds/<build-id>/...` for immutable release artifacts.
