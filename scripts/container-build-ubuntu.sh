...
"$RUNTIME" run --rm   --security-opt label=disable   -e "SUBVIRT_UBUNTU_TARGET=$TARGET_ID"   -e "SUBVIRT_UBUNTU_DIST=$SUITE"   -e "SUBVIRT_UBUNTU_LIBVIRT_SRC=$SRC_DIR"   -v "$(pwd):/work"   -w /work   "$IMAGE"   bash -lc './scripts/refresh-locked-libvirt-sources.sh "$SUBVIRT_UBUNTU_TARGET" && apt-get update && mk-build-deps -i -r -t "apt-get -y --no-install-recommends" "$SUBVIRT_UBUNTU_LIBVIRT_SRC/debian/control" && rm -rf dist && mkdir -p dist && ./scripts/build-provider-deb.sh && SUBVIRT_NATIVE_BUILD=1 ./scripts/build-libvirt-deb.sh && ./scripts/build-virt-manager-deb.sh'
